extends Node3D
class_name Chunk

const CHUNK_WIDTH: int = 8
const CHUNK_DEPTH: int = 8

var pillar_scene := preload("res://entities/chunks/pillar.tscn")

var address: String


func _ready():
	var ctx := HashingContext.new()
	ctx.start(HashingContext.HASH_SHA256)
	ctx.update(address.to_utf8_buffer())
	var digest := ctx.finish()

	for i in range(0, CHUNK_WIDTH):
		for j in range(0, CHUNK_DEPTH):
			# index into the hash by doubles
			@warning_ignore("integer_division")
			var nibble := digest[(i * CHUNK_DEPTH + j) / 2]
			if j % 2 == 0:
				# take first half of the byte
				nibble = nibble >> 4
			else:
				# take second half of the byte
				nibble = nibble & 0x0F

			var pillar: Pillar = pillar_scene.instantiate()
			pillar.name = str(i) + "_" + str(j)
			pillar.position.x = 4 + i * Pillar.PILLAR_WIDTH
			pillar.position.z = 4 + j * Pillar.PILLAR_DEPTH
			pillar.datum = nibble
			pillar.delay = (abs(CHUNK_WIDTH / 2.0 - i) + abs(CHUNK_DEPTH / 2.0 - j)) * 200
			add_child(pillar)
